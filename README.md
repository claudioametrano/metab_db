...
$ singularity shell --bind "$(pwd)":/in  fastqc:0.12.1--hdfd78af_0https://depot.galaxyproject.org/singularity/fastqc:0.12.1--hdfd78af_0
 
$ fastqc /in/data/16S_biochar_run2_1perc/*.gz -o /in/results/fastqc_raw_out --threads 4 --nogroup
...
```bash
$ singularity exec --bind "$(pwd)":/in fastqc\:0.12.1--hdfd78af_0 fastqc /in/data/16S_biochar_run2_1perc/*.gz -o /in/results/fastqc_raw_out --threads 4 --nogroup
```
...
```bash
IN_DIR="/in/data/16S_biochar_run2_1perc"
OUT_DIR="/in/results/trimmed_fastq"
OUT_REPORT="/in/results/report_fastp"
...
$ singularity exec --bind "$(pwd)":/in fastqc\:0.12.1--hdfd78af_0 fastqc /in/results/trimmed_fastq/*.gz -o /in/results/fastqc_trimmed_out --threads 4 --nogroup
```

```bash
$ mkdir ./results/multiqc_trimmed_out

$ singularity exec --bind "$(pwd)":/in multiqc\:1.26--pyhdfd78af_0 multiqc /in/results/fastqc_trimmed_out/ -o /in/results/multiqc_trimmed_out
```
...
$ singularity exec  --bind "$(pwd)":/in --home "$(pwd)":/home/qiime2 amplicon_2024.1 qiime --help
...
$ singularity exec --bind "$(pwd)":/in --home "$(pwd)":/home/qiime2 amplicon_2024.10.sif qiime tools import   --type 'SampleData[PairedEndSequencesWithQuality]'   --input-path /in/results/trimmed_fastq/ --input-format CasavaOneEightSingleLanePerSampleDirFmt --output-path /in/results/qiime_artifacts/16S_biochar.qza
```
...
$ singularity exec --bind "$(pwd)":/in rename\:1.601--hdfd78af_1 rename 's/.trimmed.fastq.gz/_001.fastq.gz/' /in/results/trimmed_fastq/*.gz
```
...
```bash
$ singularity exec --bind "$(pwd)":/in --home "$(pwd)":/home/qiime2 amplicon_2024.10.sif qiime dada2 denoise-paired \
  --p-n-threads 4\
  --p-trunc-len-f 0 \
  --p-trunc-len-r 0 \
  --i-demultiplexed-seqs /in/results/qiime_artifacts/16S_biochar.qza \
  --o-representative-sequences /in/results/qiime_artifacts/rep-seqs.qza \
  --o-table /in/results/qiime_artifacts/asv-table.qza \
  --o-denoising-stats /in/results/qiime_artifacts/stats.qza \
  --p-min-overlap 12 \
...
```bash
$ singularity exec  --bind "$(pwd)":/in --home "$(pwd)":/home/qiime2 amplicon_2024.10.sif qiime feature-table summarize \
  --i-table /in/results/qiime_artifacts/asv-table.qza \
  --o-visualization /in/results/qiime_artifacts/asv-table.qzv \
  --m-sample-metadata-file /in/data/metadata.csv

$ singularity exec --bind "$(pwd)":/in --home "$(pwd)":/home/qiime2 amplicon_2024.10.sif qiime feature-table tabulate-seqs \
  --i-data /in/results/qiime_artifacts/rep-seqs.qza \
  --o-visualization /in/results/qiime_artifacts/rep-seqs.qzv
```
...
$ singularity exec  --bind "$(pwd)":/in --home "$(pwd)":/home/qiime2 amplicon_2024.10.sif biom convert -i ./results/qiime_artifacts/name_of_the_folder/data/feature-table.biom -o ./results/asv-table.tsv --to-tsv
...
```
### TASK6 
> On the basis of the previous reports stat.qza, select suitable resampling depth and step values for the rarefaction analysis (hint: search in `--help` of the `diversity alpha-rarefaction` command)
...
```bash
$ singularity exec --bind "$(pwd)":/in --home "$(pwd)":/home/qiime2 amplicon_2024.10.sif qiime feature-classifier fit-classifier-naive-bayes \
  --i-reference-reads /in/data/silva-138-99-seqs.qza \
  --i-reference-taxonomy /in/data/silva-138-99-tax.qza \
  --o-classifier /in/results/qiime_artifacts/classifier_silva138_99.qza
```
...
```bash
$ singularity exec --bind "$(pwd)":/in --home "$(pwd)":/home/qiime2 amplicon_2024.10.sif qiime feature-classifier classify-sklearn \
  --p-n-jobs 4 \
  --i-classifier /in/results/qiime_artifacts/classifier_silva138_99.qza \
  --i-reads /in/results/qiime_artifacts/rep-seqs.qza \
  --o-classification /in/results/qiime_artifacts/taxonomy.qza
```
#### ...and plot samples composition in term of main taxa
```bash
$ singularity exec --bind "$(pwd)":/in --home "$(pwd)":/home/qiime2 amplicon_2024.10.sif qiime taxa barplot \
  --i-table /in/results/qiime_artifacts/asv-table.qza \
  --i-taxonomy /in/results/qiime_artifacts/taxonomy.qza \
  --m-metadata-file /in/data/sample-metadata.tsv \
  --o-visualization /in/results/qiime_artifacts/taxa-bar-plots.qzv
```
### TASK 7
...
--o-filtered-table /in/results/qiime_artifacts/asv-table_mito_chl_filtered.qza \
--verbose
```
...
$ singularity exec --bind "$(pwd)":/in --home "$(pwd)":/home/qiime2 amplicon_2024.10.sif qiime diversity core-metrics-phylogenetic \
  ...
  --i-table /in/results/qiime_artifacts/asv-table_mito_chl_filtered.qza \
